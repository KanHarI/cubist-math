...
if (args.includes("--help")) {
  console.log("Usage: npm run format:mathscript -- [--check] [file.cubist ...]\nWith no files, format every web/proofs/*.cubist source, including AST-checked tuple linearization. --check reports changes without writing.");
} else {
  const files = args.filter(a => a !== "--check");
  if (files.some(a => a.startsWith("--"))) throw new Error("Unknown formatter option.");
  if (!files.length) files.push(...(await readdir(new URL("../web/proofs/", import.meta.url))).filter(n => n.endsWith(".cubist")).sort().map(n => new URL("../web/proofs/" + n, import.meta.url)));
  let changed = 0;
