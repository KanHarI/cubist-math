...
process.stdout.write(JSON.stringify(parse(source, metadata)));"""
    ...
    for name in ['wnat_equiv', 'primes']:
        if (OUT / f'{name}_entry.json').exists():
            catalogue.append(json.loads((OUT / f'{name}_entry.json').read_text()))
    (OUT / 'catalogue.mjs').write_text('// Generated by make proof-export.\nexport default ' + json.dumps(catalogue, indent=2) + ';\n')
