...
process.stdout.write(JSON.stringify(parse(source, metadata)));"""
    ...
    if (OUT / 'wnat_equiv_entry.json').exists():
        catalogue.append(json.loads((OUT / 'wnat_equiv_entry.json').read_text()))
    (OUT / 'catalogue.mjs').write_text('// Generated by tools/export_workbench.py and tools/proofs/wnat_equiv.mjs.\nexport default ' + json.dumps(catalogue, indent=2) + ';\n')
