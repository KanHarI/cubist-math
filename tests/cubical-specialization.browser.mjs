...
try {
  ...
  await page.waitForFunction(() => document.querySelectorAll("#kernel-axioms-list li").length === 5);
  assert.equal(await page.locator("#kernel-context-note").textContent(), "Empty context");
  assert.equal(await page.locator("#kernel-context-list li").count(), 0);
  assert.equal(await page.locator("#kernel-axioms-label").textContent(), "Axioms");
  assert.equal(await page.locator("#kernel-axioms-list li").count(), 5);
  assert.match(await page.locator("#kernel-axioms-list").textContent(), /Choice\(U0\)/);
  await page.locator("#kernel-view").selectOption("raw");
  assert.equal(await page.locator("#kernel-context-list li").count(), 0);
  assert.equal(await page.locator("#kernel-axioms-list li").count(), 5);
  await page.locator("#kernel-view").selectOption("notation");
  await page.locator('#kernel-axioms-list button[data-name="Choice(U0)"]').click();
  ...
  assert.equal(await workbench.locator("#context").textContent(), "Empty context");
  assert.equal(await workbench.locator("#axioms-heading").textContent(), "Axioms");
  assert.match(await workbench.locator("#axioms").textContent(), /Choice\(U0\)/);
