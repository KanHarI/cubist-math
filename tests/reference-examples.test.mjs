...
<pre data-check="reject">def wrong : 0 = 1 { exact refl(0); }  // Error: Type mismatch.</pre>
...
<pre><code>def unmarked = 0;</code></pre>`;
  ...
  await assert.rejects(verify({ ...rejected, text: "def wrong : 0 = 1 { exact refl(0); }  // Error: Type mismatch.\ndef later = missing;" }, 7),
    /is not stated in a comment/);
  await assert.rejects(verify({ ...accepted, text: "def one = succ(0);  // Error: Type mismatch." }, 8), /states no error/);
