...
import {syntaxGraphBudget,syntaxNames as collectSyntaxNames,mapSyntax} from './syntax-graph.mjs';
...
function fresh(base,...terms) {
  const used=collectSyntaxNames(terms);
  while(used.has(base))base+='_';
  return base;
}
export {collectSyntaxNames as syntaxNames};
export function withFreshSyntaxNames(terms,build,{reserved=[],...budget}={}) {
  const used=collectSyntaxNames(terms,budget);
...
export function withNativeReferences(inputs,build,{deadline=performance.now()+1000,checkDeadline}={}) {
  const budget=syntaxGraphBudget({deadline,checkDeadline}),
    used=collectSyntaxNames(inputs,{budget}),references=new Map();
  let referenceSerial=0;
  const rewrite=(root,restore=false)=>mapSyntax(root,node=>{
      if(restore&&node.tag==='Var'&&references.has(node.name)) {
        return references.get(node.name);
      }
