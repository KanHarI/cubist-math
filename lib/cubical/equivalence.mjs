...
// Source terms may be small DAGs with enormous expanded trees. Scan each
// object once and poll throughout proof construction, including arrays.
export function syntaxNames(terms,{deadline=performance.now()+1000,checkDeadline}={}) {
  const used=new Set(),seen=new WeakSet(),pending=[terms];
  let visits=0;
  while(pending.length) {
    const value=pending.pop();
    if(typeof value==='string') {used.add(value.replace(/:[01]$/,''));continue;}
    if(!value||typeof value!=='object'||seen.has(value))continue;
    seen.add(value);
    if(++visits>250000)throw Error('Cubical syntax construction term-size budget exceeded.');
    if((visits&255)===1) {
...
export function withFreshSyntaxNames(terms,build,{reserved=[],...budget}={}) {
  const used=syntaxNames(terms,budget);
...
export function withNativeReferences(inputs,build,{deadline=performance.now()+1000,checkDeadline}={}) {
  const budget={deadline,checkDeadline},used=syntaxNames(inputs,budget),references=new Map();
  let visits=0,referenceSerial=0;
  const rewrite=(root,restore=false)=>{
    const memo=new WeakMap(),pending=[];
    const copy=node=>{
      if(!node||typeof node!=='object')return node;
      if(memo.has(node))return memo.get(node);
      if(++visits>500000)throw Error('Cubical syntax construction term-size budget exceeded.');
      if((visits&255)===1) {
        if(performance.now()>deadline)throw Error('Cubical syntax construction elapsed-work budget exceeded.');
        checkDeadline?.();
      }
      if(restore&&node.tag==='Var'&&references.has(node.name)) {
        const original=references.get(node.name);
        memo.set(node,original);
        return original;
      }
      if(!restore&&['Ref','DefRef'].includes(node.tag)) {
        let name;
        do {name=`checked_reference_${++referenceSerial}`;} while(used.has(name));
        used.add(name);
        const variable=v(name);
        references.set(name,node);
        memo.set(node,variable);
        return variable;
      }
      const result=Array.isArray(node)?[]:{};
      memo.set(node,result);pending.push([node,result]);
      return result;
    };
    const result=copy(root);
    while(pending.length) {
      const [node,target]=pending.pop();
      for(const [key,value] of Object.entries(node))target[key]=copy(value);
    }
    return result;
  };
  const symbolic=rewrite(inputs);
