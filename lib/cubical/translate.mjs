...
export class Translator {
  ...
  blockBody(statements,goal,ctx,env) {
    ...
    if(first.kind==="simpaOnly") {
      ...
      const deadline=performance.now()+1000;
      if(this.checker.nf(goal).tag!=="Path") {
        const supplied=this.simplifyTypeTerm(this.checker.infer(value,ctx).type,rules,ctx,deadline);
  ...
  simplifyTypeTerm(term,rules,ctx,deadline=performance.now()+1000) {
    ...
    while(true) {
      ...
      for(const rule of rules) {
        try {
          const result=rewriteFirst(this.checker,current,carrier,rule,ctx,1,
            "postorder",deadline,this.rewriteWork);
          selected={rule,result};break;
        } catch(error) {
        ...
        for(const rule of rules) {
          try {
            const result=rewriteFirst(this.checker,side==="lhs"?path.left:path.right,
              path.family,rule,ctx,1,"postorder",deadline,this.rewriteWork);
