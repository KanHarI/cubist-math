...
export class Translator {
  ...
  selectedSimpRules(items,scope,only,without=[],witnessNames=[]) {
    ...
    const solvePremise=(premise,owner,deadline,at)=>{
      ...
      try {
        ...
        const simplified=this.simplifyEqualityGoal(premise,available,at,deadline);
        at.unit.work.premiseRewriteSteps+=simplified.rewrites;
        const residual=at.nf(simplified.current);
        if(residual.tag==="Path"&&!freeDimensions(residual.family).has(residual.dim)
          ...
          const base=at.check(T.line(at.fresh("i"),residual.family,residual.left),
            simplified.current);
          const witness=at.check(simplified.rebuild(base),premise);
          at.unit.work.premiseProofs++;
          outcome={term:witness,rules:simplified.trace.flatMap(step=>[step.rule,...step.premiseRules])};
        }
      } catch(error) {
  ...
  referenceSimplificationSteps(first,trace,scope) {
  ...
  termBody(n,scope,expected=null) {
    ...
    switch(n.kind) {
      ...
      case "proof": {
        ...
        const term=this.block(n.statements,type,scope);
        const checked=scope.check(term,type);
        return scope.ascribe(checked,type);
      }
  ...
  block(statements,goal,scope) {
    try {return this.blockBody(statements,goal,scope);}
    catch(error) {
      throw scope.unit.locate(error,statements[0]);
    }
  }
  blockBody(statements,goal,scope) {
    ...
    if(first.kind==="calc") {
      ...
      for(const [index,step] of first.steps.entries()) {
        ...
        const raw=step.proof.kind==="block"?this.block(step.proof.body,target,scope)
        ...
        this.reference(scope,{...step,start:step.by?.start??step.start,end:step.by?.end??step.end,
          ...
          witness);
        proof=proof?this.composePaths(proof,witness,scope):witness;
        previous=right;
      }
      if(!scope.equal(previous,path.right))throw Error("calc final endpoint does not match the goal.");
      scope.check(proof,goal);
      this.reference(scope,{...first,name:"calc",end:first.start+4,
    ...
    if(first.kind==="rw") {
      ...
      for(const item of first.rules) {
        try {
          ...
          if(target==="lhs")rebuild=proof=>previous(this.composePaths(changed.witness,proof,scope));
          else {
            const inverse=equalityRule(scope,changed.witness,true).term;
            rebuild=proof=>previous(this.composePaths(proof,inverse,scope));
          }
          current=next;
        } catch(error) {
          throw unit.locate(error,item);
        }
      }
      let proof;
      if(rest.length)proof=this.block(rest,current,scope);
      else {
        const residual=scope.nf(current);
        if(!scope.equal(residual.left,residual.right))
          throw Error("rw left an unresolved equality goal; add a following proof statement.");
        proof=T.line(scope.fresh("i"),residual.family,residual.left);
        scope.check(proof,current);
      }
      const reconstructed=rebuild(proof);
      scope.check(reconstructed,goal);
      this.reference(scope,{...first,name:"rw",end:first.start+2,
    ...
    if(first.kind==="simpOnly") {
      ...
      if(first.at) {
        ...
        const originalType=scope.infer(source).type;
        const simplified=this.simplifyEqualityGoal(originalType,rules,scope);
        const proof=simplified.forward(source);
        const checked=scope.check(proof,simplified.current);
        const named=scope.ascribe(checked,simplified.current);
        const final=this.block(rest,goal,this.sourceBinding(first.as,named,scope));
        if(unit.references)this.reference(scope,{...first,name:"simp",end:first.start+4,
          role:"simplification witness",freeze:this.freezeSimplification(first,rules,simplified.trace,scope,
            (subset,at)=>{
              const replay=this.simplifyEqualityGoal(originalType,subset,at);
              return at.equal(replay.current,simplified.current)
                &&at.equal(replay.forward(source),named);
            }),
          description:`Checked simplified hypothesis copy with ${simplified.rewrites} rewrites.`},
          named);
        this.referenceSimplificationSteps(first,simplified.trace.map(step=>({...step,phase:"hypothesis"})),scope);
        return final;
      }
      if(scope.nf(goal).tag!=="Path") {
        const simplified=this.simplifyTypeTerm(goal,rules,scope);
        if(!rest.length)throw Error("simp left a type goal; add a following proof statement.");
        const proof=this.block(rest,simplified.current,scope);
        const reconstructed=simplified.witness
          ? this.transportTypePath(equalityRule(scope,simplified.witness,true).term,proof,scope)
          : proof;
        scope.check(reconstructed,goal);
        if(unit.references)this.reference(scope,{...first,name:"simp",end:first.start+4,
          role:"simplification witness",freeze:this.freezeSimplification(first,rules,simplified.trace,scope,
            (subset,at)=>{
              const replay=this.simplifyTypeTerm(goal,subset,at);
              if(!at.equal(replay.current,simplified.current))return false;
              const candidate=replay.witness
                ?this.transportTypePath(equalityRule(at,replay.witness,true).term,proof,at)
                :proof;
              return at.equal(candidate,reconstructed);
            }),
          description:`Checked type simplification with ${simplified.rewrites} rewrites.`},
          reconstructed);
        this.referenceSimplificationSteps(first,simplified.trace.map(step=>({...step,phase:"goal"})),scope);
        return reconstructed;
      }
      const {current,rebuild,rewrites,trace,failures,dependent}=this.simplifyEqualityGoal(goal,rules,scope);
      let proof;
      if(rest.length)proof=this.block(rest,current,scope);
      else {
        const residual=scope.nf(current);
        if(!scope.equal(residual.left,residual.right)) {
          ...
          throw unit.locate(error,blocked?.rule.sourceStart===undefined?first:
      ...
      scope.check(reconstructed,goal);
      if(unit.references)this.reference(scope,{...first,name:"simp",end:first.start+4,
        role:"simplification witness",freeze:this.freezeSimplification(first,rules,trace,scope,
          (subset,at)=>{
            const replay=this.simplifyEqualityGoal(goal,subset,at);
            return at.equal(replay.current,current)
              &&at.equal(replay.rebuild(proof),reconstructed);
          }),
        ...
        reconstructed);
      this.referenceSimplificationSteps(first,trace,scope);
      return reconstructed;
    }
    if(first.kind==="simpaOnly") {
      ...
      const deadline=searchDeadline();
      if(scope.nf(goal).tag!=="Path") {
        const supplied=this.simplifyTypeTerm(suppliedType,rules,scope,deadline);
        const target=this.simplifyTypeTerm(goal,rules,scope,deadline);
        const moved=supplied.witness
          ? this.transportTypePath(supplied.witness,value,scope):value;
        const checked=this.checkSimplified(moved,target.current,scope,supplied.dependent||target.dependent);
        const result=target.witness
          ? this.transportTypePath(equalityRule(scope,target.witness,true).term,checked,scope)
          : checked;
        scope.check(result,goal);
        const trace=[...supplied.trace.map(step=>({...step,phase:"supplied"})),
          ...target.trace.map(step=>({...step,phase:"goal"}))];
        if(unit.references)this.reference(scope,{...first,name:"simpa",end:first.start+5,
          role:"simplification witness",freeze:this.freezeSimplification(first,rules,trace,scope,
            (subset,at)=>{
              const replaySupplied=this.simplifyTypeTerm(suppliedType,subset,at);
              const replayTarget=this.simplifyTypeTerm(goal,subset,at);
              if(!at.equal(replaySupplied.current,supplied.current)
                ||!at.equal(replayTarget.current,target.current))return false;
              const replayMoved=replaySupplied.witness
                ?this.transportTypePath(replaySupplied.witness,value,at):value;
              const replayChecked=at.check(replayMoved,replayTarget.current);
              const candidate=replayTarget.witness
                ?this.transportTypePath(equalityRule(at,replayTarget.witness,true).term,
                  replayChecked,at)
                :replayChecked;
              return at.equal(candidate,result);
            }),
      ...
      const supplied=this.simplifyEqualityGoal(suppliedType,rules,scope,deadline);
      const target=this.simplifyEqualityGoal(goal,rules,scope,deadline);
      const simplified=supplied.forward(value);
      const checked=this.checkSimplified(simplified,target.current,scope,supplied.dependent||target.dependent);
      ...
      if(unit.references)this.reference(scope,{...first,name:"simpa",end:first.start+5,
        role:"simplification witness",freeze:this.freezeSimplification(first,rules,trace,scope,
          (subset,at)=>{
            const replaySupplied=this.simplifyEqualityGoal(suppliedType,subset,at);
            const replayTarget=this.simplifyEqualityGoal(goal,subset,at);
            if(!at.equal(replaySupplied.current,supplied.current)
              ||!at.equal(replayTarget.current,target.current))return false;
            const replayChecked=at.check(replaySupplied.forward(value),
    ...
    if(first.kind==="intro") {
      ...
      return T.lam(name,pi.domain,this.block(rest,bodyGoal,inner));
    }
    if(first.kind==="ext") {
      const path=scope.nf(goal);
      if(path.tag!=="Path"||freeDimensions(path.family).has(path.dim))
      ...
      const pointwise=T.path(scope.fresh("i"),output,T.app(path.left,variable),T.app(path.right,variable));
      const body=this.block(rest,pointwise,inner);
    ...
    if(first.kind==="over") {
      ...
      const obligation=T.path(scope.fresh("j"),fiber,transported,target.right);
      const proof=this.block(first.body,obligation,scope);
      const checked=scope.check(proof,obligation);
    ...
    if(first.kind==="let"&&first.target.kind==="name") {
      ...
      scope.infer(value);
      return this.block(rest,goal,this.sourceBinding(first.target,value,scope));
    }
    if(first.kind==="obtain") {
      ...
      return this.block(rest,goal,inner);
    }
    if(first.kind==="cases") {
      ...
      const motive=T.lam(scope.fresh(),type,goal);
      const branch=side=>{
        const name=scope.fresh(),domain=sum[side];
        const inner=this.sourceBinding(first[side],T.variable(name),scope.bind(name,domain));
        return T.lam(name,domain,this.block(first[side+"Body"],goal,inner));
      };
      return T.sumrec(motive,branch("left"),branch("right"),value);
    }
    if(first.kind==="have") {
      const type=this.term(first.type,scope,null),value=this.block(first.body,type,scope);
      ...
      return this.block(rest,goal,this.sourceBinding(first.name,named,scope));
    }
  ...
  // Rebuild a proof from checked steps after a search or calc chain. The number
  // of compositions is bounded by the rewrites or steps, so only declaration
  // cancellation applies here, never a tactic's search time limit.
  composePaths(left,right,scope) {
    scope.checkDeadline();
  ...
  transportTypePath(path,value,scope) {
  ...
  simplifyTypeTerm(term,rules,scope,deadline=searchDeadline()) {
    const carrier=scope.nf(scope.infer(term).type);
    if(carrier.tag!=="U")throw Error("General simplification requires a type-valued goal.");
    let current=term,rewrites=0,dependent=false;
    ...
    while(true) {
      ...
      if(!found)break;
      if(++rewrites>64)throw new SearchLimit("Simplification rewrite budget exceeded.");
      current=found.after;
    ...
    let witness=null;
    for(const step of trace)witness=witness?this.composePaths(witness,step.witness,scope):step.witness;
    return {current,witness,rewrites,trace,dependent};
  }
  simplifyEqualityGoal(goal,rules,scope,deadline=searchDeadline()) {
    let current=goal,rebuild=proof=>proof,forward=proof=>proof,rewrites=0,dependent=false;
    const trace=[];
    let failures=[];
    ...
    while(true) {
      ...
      for(const side of ["lhs","rhs"]) {
        ...
        dependent||=unsupported;
        if(found) {selected={side,result:found,rule:found.rule};break;}
      }
      if(!selected) {
      ...
      current=T.path(scope.fresh("i"),path.family,
        side==="lhs"?result.after:path.left,
        side==="rhs"?result.after:path.right);
      // These closures run after the search, possibly after later statements.
      // They compose checked paths and carry no search time limit.
      if(side==="lhs") {
        rebuild=proof=>previous(this.composePaths(result.witness,proof,scope));
        forward=proof=>this.composePaths(equalityRule(scope,result.witness,true).term,
          previousForward(proof),scope);
      } else {
        const inverse=equalityRule(scope,result.witness,true).term;
        rebuild=proof=>previous(this.composePaths(proof,inverse,scope));
        forward=proof=>this.composePaths(previousForward(proof),result.witness,scope);
      }
    }
    // `dependent` describes the final pass: a remaining match was skipped.
    return {current,rebuild,forward,rewrites,trace,failures,dependent};
  }
}
