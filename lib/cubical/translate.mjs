...
export class Translator {
  ...
  selectedSimpRules(items,ctx,env,only,without=[],witnessNames=[]) {
    ...
    for(const item of items) {
      ...
      if(set)selected.push(...set);
        ...
        const term=this.term(item.value,ctx,env,null);
    ...
    return uniqueSimpRules(selected.filter(rule=>!excluded.has(rule.identity)))
      .map(rule=>{
        const compiled=simplificationRule(this.checker,rule.term,ctx,rule.reverse,witnesses);
  ...
  freezeSimplification(first,rules,trace) {
    if(first.only)return null;
    const used=new Set(trace.map(step=>`${step.rule.identity}:${step.rule.reverse}`));
  ...
  referenceSimplificationSteps(first,trace,ctx,env) {
    for(const [index,step] of trace.entries()) {
      ...
      this.onReference?.({name:`simp step ${index+1}`,start:first.start,
        ...
        description:`${step.phase??"goal"} ${step.side}: checked rewrite ${index+1} using ${step.rule.reverse?"<- ":""}${rule}; ${step.visits} nodes visited.`},
  ...
  block(statements,goal,ctx,env) {
    try {return this.blockBody(statements,goal,ctx,env);}
      ...
      if(!Number.isInteger(error.offset)&&Number.isInteger(statements[0]?.start)) {
        error.offset=statements[0].start;
        const before=this.source.slice(0,error.offset),line=before.split("\n").length;
  ...
  blockBody(statements,goal,ctx,env) {
    ...
    if(first.kind==="rw") {
      ...
      for(const item of first.rules) {
        ...
        if(target) changed=rewriteFirst(this.checker,path[target==="lhs"?"left":"right"],path.family,rule,ctx,first.occurrence,"preorder",deadline);
        else {
          try {changed=rewriteFirst(this.checker,path.left,path.family,rule,ctx,first.occurrence,"preorder",deadline);target="lhs";}
          catch(error) {
            if(error.code!=="REWRITE_OCCURRENCE_MISSING")throw error;
            changed=rewriteFirst(this.checker,path.right,path.family,rule,ctx,
              first.occurrence-error.eligible,"preorder",deadline);target="rhs";
          }
        }
    ...
    if(first.kind==="simpOnly") {
      ...
      const {current,rebuild,rewrites,trace}=this.simplifyEqualityGoal(goal,rules,ctx);
      let proof;
      if(rest.length)proof=this.block(rest,current,ctx,env);
      else {
        const residual=this.checker.nf(current);
        if(!this.checker.equal(residual.left,residual.right,ctx))
          throw Error("simp only left an unresolved equality goal; add a following proof statement.");
        proof=T.line(this.fresh("i"),residual.family,residual.left);
        this.checker.check(proof,current,ctx);
      }
      const reconstructed=rebuild(proof);
    ...
    if(first.kind==="simpaOnly") {
      ...
      const deadline=performance.now()+1000;
      const supplied=this.simplifyEqualityGoal(this.checker.infer(value,ctx).type,rules,ctx,deadline);
  ...
  simplifyEqualityGoal(goal,rules,ctx,deadline=performance.now()+1000) {
        ...
        for(const rule of rules) {
          try {
            const result=rewriteFirst(this.checker,side==="lhs"?path.left:path.right,
              path.family,rule,ctx,1,"postorder",deadline);
            selected={side,result,rule};break;
          } catch(error) {
            if(!error.message.startsWith("Rewrite occurrence"))throw error;
          }
        }
        if(selected)break;
      }
    ...
    return {current,rebuild,forward,rewrites,trace};
  }
}
