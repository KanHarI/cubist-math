...
export class Translator {
  ...
  evaluate(d,scope) {
    ...
    if(!scope.equal(result.type,expected.type))
      throw Error(`The evaluated term has type ${sourceText(result.type)}, but the expected value has type ${sourceText(expected.type)}.`);
    const text=term=>{ const shown=sourceText(term); return shown.length>200?`${shown.slice(0,200)}…`:shown; };
