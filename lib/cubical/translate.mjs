...
export class Translator {
  ...
  schema(expression,env,simpRegistry=this.simpRegistry,moduleName=this.moduleName) {
    if(expression.kind==="binderGroup"&&expression.binderKind==="lambda"
      &&expression.domain.kind==="name"&&expression.domain.name==="Universe") {
      let expanded=expression.body;
      for(const name of [...expression.names].reverse())
        expanded={kind:"lambda",name,domain:expression.domain,body:expanded};
      return this.schema(expanded,env,simpRegistry,moduleName);
    }
