...
const boundedStateKey = (term,deadline,kernel) => {
  ...
  const state=JSON.stringify(term,(key,value)=>{
    ...
    if(estimated>250000)throw Error("Simplification term-size budget exceeded.");
    if((++visits&255)===0) {
...
export class Translator {
  ...
  registeredSimpRule(token,env,priority=0) {
    ...
    return {term,identity:boundedStateKey(term,performance.now()+1000,this.checker.kernel),origin:this.moduleName,priority,
  ...
  selectedSimpRules(items,ctx,env,only,without=[],witnessNames=[]) {
    ...
    for(const item of items) {
      ...
      if(set)selected.push(...set.map(rule=>({...rule,
        ...
        selected.push({term,identity:boundedStateKey(term,deadline,this.checker.kernel),reverse:item.reverse,
      ...
      .map(rule=>{
        ...
        if(!sourceText&&available) {
          try {
            ...
            if(!/Simplification (term-size|elapsed-work) budget exceeded\./.test(error.message))throw error;
          }
        }
  ...
  blockBody(statements,goal,ctx,env) {
    ...
    if(first.kind==="calc") {
      ...
      for(const [index,step] of first.steps.entries()) {
        if(performance.now()>deadline)throw Error("Calculation elapsed-work budget exceeded.");
        this.checker.kernel?.checkDeadline();
        ...
        proof=proof?this.composePaths(proof,witness,ctx,deadline):witness;
        previous=right;
      }
      ...
      for(const item of first.rules) {
        try {
          ...
          if(target) changed=rewriteFirst(this.checker,path[target==="lhs"?"left":"right"],path.family,rule,ctx,first.occurrence,"preorder",deadline,this.rewriteWork);
            ...
            try {changed=rewriteFirst(this.checker,path.left,path.family,rule,ctx,first.occurrence,"preorder",deadline,this.rewriteWork,true);target="lhs";}
            catch(error) {
              if(error.code!=="REWRITE_OCCURRENCE_MISSING")throw error;
              try {
                changed=rewriteFirst(this.checker,path.right,path.family,rule,ctx,
                  first.occurrence-error.eligible,"preorder",deadline,this.rewriteWork,true);
                target="rhs";
              } catch(rightError) {
                if(rightError.code==="REWRITE_OCCURRENCE_MISSING"
                  &&(error.unsupported||rightError.unsupported))
                  throw Error("Rewrite match occurs in an unsupported dependent position.");
                if(rightError.code==="REWRITE_OCCURRENCE_MISSING") {
                  rightError.eligible+=error.eligible;
                  rightError.message=`Rewrite occurrence ${first.occurrence} was not found (${rightError.eligible} eligible matches).`;
                }
                throw rightError;
              }
            }
          ...
          if(target==="lhs")rebuild=proof=>previous(this.composePaths(changed.witness,proof,ctx,deadline));
          else {
            const inverse=equalityRule(this.checker,changed.witness,ctx,true).term;
            rebuild=proof=>previous(this.composePaths(proof,inverse,ctx,deadline));
          }
          current=next;
        } catch(error) {
  ...
  composePaths(left,right,ctx,deadline=performance.now()+1000) {
    if(performance.now()>deadline)throw Error("Path composition elapsed-work budget exceeded.");
    this.checker.kernel?.checkDeadline();
    ...
    const raw=pathConcat(p.family,p.left,left,right,
      {deadline,checkDeadline:()=>this.checker.kernel?.checkDeadline()});
    const checked=this.checker.check(raw,target,ctx);
  ...
  simplifyTypeTerm(term,rules,ctx,deadline=performance.now()+1000) {
    ...
    while(true) {
      ...
      witness=witness?this.composePaths(witness,result.witness,ctx,deadline):result.witness;
  ...
  simplifyEqualityGoal(goal,rules,ctx,deadline=performance.now()+1000) {
    ...
    while(true) {
      ...
      if(side==="lhs") {
        rebuild=proof=>previous(this.composePaths(result.witness,proof,ctx,deadline));
        forward=proof=>this.composePaths(equalityRule(this.checker,result.witness,ctx,true).term,
          previousForward(proof),ctx,deadline);
      } else {
        const inverse=equalityRule(this.checker,result.witness,ctx,true).term;
        rebuild=proof=>previous(this.composePaths(proof,inverse,ctx,deadline));
        forward=proof=>this.composePaths(previousForward(proof),result.witness,ctx,deadline);
      }
    }
