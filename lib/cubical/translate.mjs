...
export class Translator {
  ...
  // Generated names are a source stem followed by a unique serial. A stem that
  // ends in a digit gets a separator: otherwise a1 with serial 1 and a with
  // serial 11 would both be a11, and one binder would capture the other.
  fresh(prefix="b") {
    const stem=/[0-9]$/.test(prefix)?`${prefix}_`:prefix;
    let name;do{name=`${stem}${++this.serial}`;}while(this.checker.assumptions?.has(name));return name;
  }
  ...
  bindLocal(ctx,name,type) {
    if(ctx.has(name))throw Error(`Internal elaboration error: ${name} is already bound.`);
    return new Map(ctx).set(name,type);
  }
  bindDimension(dimensions,name) {
    ...
    return new Map(dimensions).set(name,index);
  }
  ...
  dimensionBody(n,dim,ctx,env,expected=null) {
    ...
    this.dimensions=this.bindDimension(previous,dim);this.checker.dimensions=this.dimensions;
  ...
  schema(expression,env,simpRegistry=this.simpRegistry,moduleName=this.moduleName,
    ...
    return expression.kind==="lambda"&&expression.domain?.kind==="name"&&expression.domain.name==="Universe"
      ? {tag:"UniverseSchema",parameter:expression.name.text,body:expression.body,
        env:new Map(env),simpRegistry:copySimpRegistry(simpRegistry),moduleName,source} : null;
  }
  ...
  evaluate(d,env) {
    const result=this.checker.verify(this.term(d.value,new Map(),env));
    ...
    const expected=this.checker.verify(this.term(d.expected,new Map(),env));
    if(!this.checker.equal(result.type,expected.type))
    ...
    if(!this.checker.equal(result.normal,expected.normal))
      ...
      if(d.kind==="evaluate") {
        ...
        try {
          directives.push({kind:"evaluate",name:`at line ${line}`,status:"checked",start:d.start,
            normalText:this.evaluate(d,env)});
        } catch(error) {
  ...
  registeredSimpRule(token,env,priority=0) {
    ...
    return {term,identity:boundedStateKey(term,searchDeadline(),this.checker.kernel),origin:this.moduleName,priority,
    ...
    for(const token of without) {
      ...
      if(set)for(const rule of set)excluded.add(rule.identity);
        ...
        try {
          ...
          simplificationRule(this.checker,term,ctx,false,[],null,deadline);
    ...
    const solvePremise=(premise,owner,deadline)=>{
      ...
      try {
        ...
        if(residual.tag==="Path"&&!freeDimensions(residual.family).has(residual.dim)
          &&this.checker.equal(residual.left,residual.right,ctx)) {
    ...
    rules=uniqueSimpRules(selected.filter(rule=>!excluded.has(rule.identity)))
      .map(rule=>{
        ...
        try {compiled=simplificationRule(this.checker,rule.term,ctx,rule.reverse,witnesses,
          (premise,deadline)=>solvePremise(premise,rule.identity,deadline),deadline);}
  ...
  freezeSimplification(first,rules,trace,replay) {
    if(first.only||!this.freezeSuggestions)return null;
    ...
    if(frozen.some(rule=>!rule.reverse&&this.simpRegistry.sets.has(rule.sourceText)))return null;
    ...
    switch(n.kind) {
      ...
      case "binderGroup": {
        ...
        for(const token of n.names) {
          ...
          inner=this.bindLocal(inner,name,domain);scope=new Map(scope).set(token.text,variable);
      ...
      case "lambda": case "forall": case "exists": {
        ...
        const name=this.fresh(n.name.text),inner=this.bindLocal(ctx,name,domain),scope=new Map(env).set(n.name.text,T.variable(name));
      ...
      case "call": {
        ...
        for(const arg of n.args) {
          if(fn.tag==="UniverseSchema") {
            ...
            const next=this.schema(fn.body,scope,fn.simpRegistry,fn.moduleName,fn.source),
            ...
            if(next) { next.binding=fn.binding;next.levels=levels;fn=next; }
              ...
              const previousRegistry=this.simpRegistry,previousModule=this.moduleName,
                previousSource=this.source,previousFreeze=this.freezeSuggestions;
              this.simpRegistry=copySimpRegistry(schema.simpRegistry);
              this.moduleName=schema.moduleName;
              this.source=schema.source;
              // Only binder names are collected here; no edit applies to a specialization.
              this.freezeSuggestions=false;
              ...
              try {
                ...
                this.locateProofError(error,schema.body);
                ...
                this.freezeSuggestions=previousFreeze;
              }
      ...
      case "induction": {
        ...
        const stepCtx=this.bindLocal(ctx,k,T.nat), scope=new Map(env).set(n.index.text,T.variable(k));
        ...
        const hypothesisCtx=this.bindLocal(stepCtx,ih,motiveBody);scope.set(n.hypothesis.text,T.variable(ih));
        this.sourceBinding(n.hypothesis,T.variable(ih),hypothesisCtx,scope);
        const step=T.lam(k,T.nat,T.lam(ih,motiveBody,this.term(n.step,hypothesisCtx,scope,T.app(motive,T.succ(T.variable(k))))));
      ...
      case "match": {
        ...
        const name=this.fresh(),scope=new Map(env),inner=this.bindLocal(ctx,name,type);
        ...
        const branch=side=>{
          ...
          return T.lam(local,domain,this.term(n[side+"Body"],this.bindLocal(ctx,local,domain),branchEnv,T.app(motive,injection)));
        };
  ...
  blockBody(statements,goal,ctx,env) {
    ...
    if(first.kind==="calc") {
      ...
      for(const [index,step] of first.steps.entries()) {
        ...
        this.onReference?.({...step,start:step.by?.start??step.start,end:step.by?.end??step.end,
      ...
      for(const item of first.rules) {
        try {
          ...
          if(target) changed=rewriteFirst(this.checker,path[target==="lhs"?"left":"right"],path.family,rule,ctx,first.occurrence,"preorder",deadline,this.rewriteWork);
            ...
            const left=findRewrite(this.checker,path.left,path.family,rule,ctx,first.occurrence,"preorder",deadline,this.rewriteWork);
            if(left.found) {changed=left.found;target="lhs";}
            else {
              const right=findRewrite(this.checker,path.right,path.family,rule,ctx,
                first.occurrence-left.eligible,"preorder",deadline,this.rewriteWork);
    ...
    if(first.kind==="simpOnly") {
      ...
      if(this.checker.nf(goal).tag!=="Path") {
        ...
        this.onReference?.({...first,name:"simp",end:first.start+4,
          role:"simplification witness",freeze:this.freezeSimplification(first,rules,simplified.trace,
            subset=>{
              const replay=this.simplifyTypeTerm(goal,subset,ctx);
              if(!this.checker.equal(replay.current,simplified.current,ctx))return false;
              const candidate=replay.witness
                ?this.transportTypePath(equalityRule(this.checker,replay.witness,ctx,true).term,proof,ctx)
                :proof;
              return this.checker.equal(candidate,reconstructed,ctx);
            }),
      ...
      const {current,rebuild,rewrites,trace,failures,dependent}=this.simplifyEqualityGoal(goal,rules,ctx);
      let proof;
      if(rest.length)proof=this.block(rest,current,ctx,env);
      else {
        const residual=this.checker.nf(current);
      ...
      this.onReference?.({...first,name:"simp",end:first.start+4,
        role:"simplification witness",freeze:this.freezeSimplification(first,rules,trace,
          subset=>{
            const replay=this.simplifyEqualityGoal(goal,subset,ctx);
            return this.checker.equal(replay.current,current,ctx)
              &&this.checker.equal(replay.rebuild(proof),reconstructed,ctx);
          }),
    ...
    if(first.kind==="simpaOnly") {
      ...
      if(this.checker.nf(goal).tag!=="Path") {
        ...
        const checked=this.checkSimplified(moved,target.current,ctx,supplied.dependent||target.dependent);
        ...
        this.onReference?.({...first,name:"simpa",end:first.start+5,
          role:"simplification witness",freeze:this.freezeSimplification(first,rules,trace,
            subset=>{
              const replaySupplied=this.simplifyTypeTerm(suppliedType,subset,ctx);
              const replayTarget=this.simplifyTypeTerm(goal,subset,ctx);
              if(!this.checker.equal(replaySupplied.current,supplied.current,ctx)
                ||!this.checker.equal(replayTarget.current,target.current,ctx))return false;
              const replayMoved=replaySupplied.witness
                ?this.transportTypePath(replaySupplied.witness,value,ctx):value;
              const replayChecked=this.checker.check(replayMoved,replayTarget.current,ctx);
              const candidate=replayTarget.witness
                ?this.transportTypePath(equalityRule(this.checker,replayTarget.witness,ctx,true).term,
                  replayChecked,ctx)
                :replayChecked;
              return this.checker.equal(candidate,result,ctx);
            }),
      ...
      const checked=this.checkSimplified(simplified,target.current,ctx,supplied.dependent||target.dependent);
      ...
      this.onReference?.({...first,name:"simpa",end:first.start+5,
        role:"simplification witness",freeze:this.freezeSimplification(first,rules,trace,
          subset=>{
            const replaySupplied=this.simplifyEqualityGoal(suppliedType,subset,ctx);
            const replayTarget=this.simplifyEqualityGoal(goal,subset,ctx);
            if(!this.checker.equal(replaySupplied.current,supplied.current,ctx)
              ||!this.checker.equal(replayTarget.current,target.current,ctx))return false;
            const replayChecked=this.checker.check(replaySupplied.forward(value),
              replayTarget.current,ctx);
            return this.checker.equal(replayTarget.rebuild(replayChecked),result,ctx);
          }),
    ...
    if(first.kind==="intro") {
      ...
      const name=this.fresh(first.name.text),variable=T.variable(name),inner=this.bindLocal(ctx,name,pi.domain);
    ...
    if(first.kind==="ext") {
      ...
      const inner=this.bindLocal(ctx,name,pi.domain),scope=new Map(env).set(first.variable.text,variable);
    ...
    if(first.kind==="cases") {
      ...
      const branch=side=>{
        ...
        const inner=this.bindLocal(ctx,name,domain);
  ...
  composePaths(left,right,ctx) {
    ...
    const raw=pathConcat(p.family,p.left,left,right,
      {deadline:Infinity,checkDeadline:()=>this.checker.kernel?.checkDeadline()});
    const checked=this.checker.check(raw,target,ctx);
  ...
  checkSimplified(term,type,ctx,dependent) {
    try {return this.checker.check(term,type,ctx);}
    catch(error) {
  ...
  findSimplification(root,carrier,rules,ctx,deadline) {
    try {return findRewrite(this.checker,root,carrier,rules,ctx,1,"postorder",deadline,this.rewriteWork);}
    catch(error) {
  ...
  simplifyTypeTerm(term,rules,ctx,deadline=searchDeadline()) {
    ...
    while(true) {
      ...
      const {found,unsupported}=this.findSimplification(current,carrier,rules,ctx,deadline);
    ...
    let witness=null;
    for(const step of trace)witness=witness?this.composePaths(witness,step.witness,ctx):step.witness;
    return {current,witness,rewrites,trace,dependent};
  }
  simplifyEqualityGoal(goal,rules,ctx,deadline=searchDeadline()) {
    ...
    while(true) {
      ...
      for(const side of ["lhs","rhs"]) {
        ...
        const {found,unsupported}=this.findSimplification(side==="lhs"?path.left:path.right,
          path.family,rules,ctx,deadline);
        dependent||=unsupported;
