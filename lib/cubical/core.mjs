...
// `kind` classifies a failure as the native kernel does: "mismatch" when a
// type is not the expected one, "budget" when normalization runs out of fuel.
const fail = (message, kind) => { throw Object.assign(new Error(message), kind ? {kind} : {}); };
...
function free(t, dimension = false, result = new Set(), memo = new WeakMap()) {
  ...
  const local=new Set();
  if (dimension && ["PApp","PushPath"].includes(t.tag)) for (const n of I.names(t.arg)) local.add(n);
  ...
  for (const key of children[t.tag]) if (t[key]) {
    const sub = free(t[key], dimension,new Set(),memo);
    for(const n of sub) {
...
function substitute(t, n, value, dimension = false, memo = new WeakMap()) {
  ...
  for (const key of children[t.tag]) if (result[key]) {
    ...
    result[key] = substitute(result[key], n, value, dimension, memo);
  }
...
function normal(t, fuel) {
  if (--fuel.left < 0) fail("Reference normalization limit reached.", "budget");
  if(t.tag==="Unglue") {
...
export class Checker {
  ...
  expect(actual,expected) {
    ...
    fail(`Type mismatch: ${JSON.stringify(alpha(this.nf(actual)))} versus ${JSON.stringify(alpha(this.nf(expected)))}`, "mismatch");
  }
  ...
  // A speculative check, answered as a value like the native adapter's.
  attempt(t,expected,ctx,dims) {
    try { return {ok:true,term:this.check(t,expected,ctx,dims)}; }
    catch (error) { if (!["mismatch","budget"].includes(error?.kind)) throw error; return {ok:false,failure:error.kind,error}; }
  }
  infer(t,ctx=new Map(),dims=new Set()) {
    // Elaboration scopes pass their dimensions as a Map from name to slot.
    if (dims instanceof Map) dims=new Set(dims.keys());
