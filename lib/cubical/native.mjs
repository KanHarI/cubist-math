...
import {dimensionContextKey} from "./syntax-graph.mjs";
...
export function nativeRequest(term,expected=null,assumptions=[],{normalize=true,definitions=[],unfoldingHints=[]}={}) {
  ...
  // Native handles may be reused for the same syntax node in the same
  // dimension environment. Without this, a compact shared term serializes
  // as its exponentially larger unfolded tree.
  let encoded=new WeakMap(),encodedNodes=0;
  const encode=(t,dims=new Map())=>{
    if(!t)throw Error("Missing native input term.");
    let contexts=encoded.get(t);
    const contextKey=dimensionContextKey(dims);
    if(contexts?.has(contextKey))return contexts.get(contextKey);
    if(++encodedNodes>500000)throw Error("Native syntax serialization term-size budget exceeded.");
    const child=x=>encode(x,dims);
    const result=(()=>{switch(t.tag){
    ...
    if(!contexts){contexts=new Map();encoded.set(t,contexts);}
    contexts.set(contextKey,result);
    return result;
  };
  const encodeRoot=t=>{encoded=new WeakMap();encodedNodes=0;return encode(t);};
  ...
  for(const definition of definitions){
    const symbol=name(definition.name),body=encodeRoot(definition.value),type=definition.type?encodeRoot(definition.type):0;
  ...
  for(const [n,type]of assumptions){const symbol=name(n),handle=encodeRoot(type);lines.push(`A ${symbol} ${handle}`);}
  if(unfoldingHints.length)lines.push(hintLine(unfoldingHints));
  const value=encodeRoot(term),type=expected?encodeRoot(expected):0;lines.push(`Q ${value} ${type} ${normalize?1:0}`);
  const decoded=new Map();
  const decode=t=>{
    if(!t||typeof t!=="object")return t;
    if(Array.isArray(t))return t.map(decode);
    if(Object.hasOwn(t,"$ref")) {
      if(!decoded.has(t.$ref))throw Error("Invalid native syntax graph reference.");
      return decoded.get(t.$ref);
    }
    const result={};
    if(Object.hasOwn(t,"$id"))decoded.set(t.$id,result);
    for(const [key,value] of Object.entries(t))if(key!=="$id")result[key]=decode(value);
...
export function checkNative(term,expected=null,assumptions=[],{build=process.env.CUBICAL_NATIVE_BUILD??"build",normalize=true,definitions=[],unfoldingHints=[]}={}) {
  ...
  if(run.status!==0)throw Error(`Native checker process failed (${run.status ?? run.signal ?? "spawn"}): ${run.error?.message ?? run.stderr}`);
