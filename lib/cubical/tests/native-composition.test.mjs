...
import {T,Checker as ReferenceChecker} from "../core.mjs";
import {interval as I,face as F} from "../lattice.mjs";
import {checkNative} from "../native.mjs";
class Checker {
  verify(term,type=null,ctx=[]) {
    const result=checkNative(term,type,ctx);
    if(!result.ok)throw Error(result.error);
    const reference=new ReferenceChecker(),expected=reference.verify(term,type,ctx);
    assert(reference.equal(result.type,expected.type));
    assert(reference.equal(result.normal,expected.normal));
    reference.verify(result.normal,expected.type,ctx);
    return result;
  }
}
...
test("native: composition fills a box and constructs path concatenation",()=>{
...
test("native: transport through the Nat and Unit constructor computations",()=>{
  const check=new Checker();
...
test("native: faces cannot depend on the direction being composed",()=>{
  assert.throws(()=>new Checker().verify(T.comp("i",A,[{face:F.endpoint("i",0),term:a}],a),A,ctx),/unbound.*dimension i/i);
});
test("native: dimension substitution preserves a checked partial boundary",()=>{
  const check=new Checker();
...
test("native: Nat composition does not discard a nonconstructor tube",()=>{
...
test("native: composition through Path adds both endpoint faces and rechecks",()=>{
...
test("native: Sigma composition fills the first coordinate before the dependent second",()=>{
...
test("native: Pi composition fills its domain backwards and computes on numerals",()=>{
...
test("native: neutral path composition retains and rechecks both endpoint walls",()=>{
  const check=new Checker(),type=path(A,a,b);
  const result=check.verify(T.comp("i",type,[],p),type,ctx);
  assert.equal(result.normal.tag,"PLam");
  check.verify(result.normal,type,ctx);
});
test("native: function transport handles a genuinely varying domain",()=>{
  const check=new Checker(),P=v("P"),ft=T.pi("n",T.nat,T.nat);
  const local=[["P",path(T.universe(0),T.nat,T.unit)],["f",ft]];
  const family=T.pi("x",at(P,I.variable("i")),T.nat);
  const result=check.verify(T.comp("i",family,[],v("f")),T.pi("x",T.unit,T.nat),local);
  check.verify(result.normal,T.pi("x",T.unit,T.nat),local);
});
