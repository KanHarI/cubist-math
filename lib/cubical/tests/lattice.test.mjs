...
test("interval reversal and endpoint faces bound distributive expansion", () => {
  let term = I.one;
  for (let k = 0; k < 9; k++)
    term = I.meet(term, I.join(I.variable(`d${2*k}`), I.variable(`d${2*k+1}`)));
  assert.throws(() => I.reverse(term), /Cubical lattice term-size budget exceeded/);
  assert.throws(() => F.equalEndpoint(term, 0), /Cubical lattice term-size budget exceeded/);
});
