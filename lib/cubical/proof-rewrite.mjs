    ...
    for (const [index,marker] of parameters.entries()) {
      ...
      if(!arg&&premiseIndexes.has(index)) {
        for(const witness of witnesses) {
          try {arg=at.check(witness,pi.domain);break;}
  ...
  const consider = (subject, path) => {
    ...
    for(const candidate of Array.isArray(rule)?rule:[rule]) {
      ...
      try {
        ...
        try { scope.check(subject, selected.carrier); }
        ...
        try { scope.check(contextTerm, T.pi(hole, selected.carrier, carrier)); }
        catch (error) {
