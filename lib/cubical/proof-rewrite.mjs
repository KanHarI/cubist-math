...
export function simplificationRule(checker, term, context, reverse = false, witnesses = [], solvePremise = null,
  ...
  const collect=value=>{
    ...
    seen.add(value);
    if((++visits&255)===1) {
      if(performance.now()>deadline)throw Error("Simplification rule-pattern work budget exceeded.");
...
export function rewriteFirst(checker, root, carrier, rule, context, occurrence = 1,
  ...
  const consider = (subject, path) => {
    ...
    for(const candidate of Array.isArray(rule)?rule:[rule]) {
      ...
      if (++candidates > 8192) throw Error("Rewrite candidate budget exceeded.");
  ...
  visit(root);
  if (!found && unsupported && eligible < occurrence && !deferUnsupported)
