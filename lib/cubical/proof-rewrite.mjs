...
export function simplificationRule(checker, term, context, reverse = false, witnesses = []) {
  ...
  let failure=null;
  return {clearDiagnostic() {failure=null;},diagnostic() {return failure;},instantiate(subject) {
    ...
    for (const [index,marker] of parameters.entries()) {
      ...
      checker.check(arg,pi.domain,context);
      applied = T.app(applied,arg);
    }
    const concrete = equalityRule(checker,applied,context,reverse);
    return checker.equal(subject,concrete.left,context) ? concrete : null;
  } };
}
...
export function rewriteFirst(checker, root, carrier, rule, context, occurrence = 1,
  ...
  let visits = 0, eligible = 0, unsupported = false;
  let found = null;
  const consider = (subject, path) => {
    if (++visits > 512) throw Error("Rewrite traversal budget exceeded.");
    if (performance.now() > deadline) throw Error("Rewrite elapsed-work budget exceeded.");
    if (visits % 16 === 0) checker.kernel?.checkDeadline();
    const selected = rule.instantiate?.(subject) ?? (rule.instantiate ? null : rule);
    if (!selected) return;
    try { checker.check(subject, selected.carrier, context); }
    catch (error) { if (error.message === "Type mismatch.") return; throw error; }
    if (!checker.equal(subject, selected.left, context)) return;
  ...
  visit(root);
  if (!found && unsupported && eligible < occurrence)
    throw Error("Rewrite match occurs in an unsupported dependent position.");
  if (!found) {
    const error=Error(`Rewrite occurrence ${occurrence} was not found (${eligible} eligible matches).`);
    error.code="REWRITE_OCCURRENCE_MISSING";
