...
const root=new URL("../../archive/first-library/",import.meta.url),modules=new Map(),results=new Map(),active=new Set();
for(const file of (await readdir(root)).filter(n=>n.endsWith(".cubist")&&!n.endsWith(".construction.cubist")).sort()) {
  const source=await readFile(new URL(file,root),"utf8");modules.set(file.slice(0,-".cubist".length),{source,ast:parse(source)});
}
function visit(name) {
...
const sourceRevision=execFileSync("git",["log","-1","--format=%H","--","archive/first-library"],{cwd:fileURLToPath(new URL("../../",import.meta.url)),encoding:"utf8"}).trim();
